...
async function create(context, listId, entity, files) {
    const res = await knex.transaction(async tx => {
        ...
        await activityLog.logEntityActivity(context, LogTypeId.LIST, ListActivityType.CREATE_IMPORT, listId, {importId: id, importStatus: entity.status});
...
async function updateWithConsistencyCheck(context, listId, entity) {
    await knex.transaction(async tx => {
        ...
        await activityLog.logEntityActivity(context, LogTypeId.LIST, ListActivityType.UPDATE_IMPORT, listId, {importId: entity.id, importStatus: entity.status});
    });
}
...
async function removeTx(tx, context, listId, id) {
    ...
    await activityLog.logEntityActivity(context, LogTypeId.LIST, ListActivityType.REMOVE_IMPORT, listId, {importId: id});
}
...
async function start(context, listId, id) {
    await knex.transaction(async tx => {
        ...
        await activityLog.logEntityActivity(context, LogTypeId.LIST, ListActivityType.IMPORT_STATUS_CHANGE, listId, {importId: id, importStatus: ImportStatus.RUN_SCHEDULED});
    });
...
async function stop(context, listId, id) {
    await knex.transaction(async tx => {
        ...
        await activityLog.logEntityActivity(context, LogTypeId.LIST, ListActivityType.IMPORT_STATUS_CHANGE, listId, {importId: id, importStatus: ImportStatus.RUN_STOPPING});
    });
